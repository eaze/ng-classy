...
function Component (componentName, options) {
  return function (Class) {
    if (typeof componentName === 'object') {
      options = componentName
      componentName = options.selector ? dashCaseToCamelCase(options.selector) : pascalCaseToCamelCase(Class.name)
    } else {
      componentName = pascalCaseToCamelCase(componentName)
    }

    options || (options = {})

    options.bindings = options.bindToController || options.bindings || options.bind || {}

    // Component's second parameter is an object, not a function
    app.component(componentName,
      window.angular.merge({}, Component.defaults, {
        controller: Class
      }, options || {})
    )

    if (Class.$initState) {
      Class.$initState(componentName)
    }
